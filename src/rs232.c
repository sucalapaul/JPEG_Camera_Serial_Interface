...
char comports[36][16]={"/dev/ttyS0","/dev/ttyS1","/dev/ttyS2","/dev/ttyS3","/dev/ttyS4","/dev/ttyS5",
                       "/dev/ttyS6","/dev/ttyS7","/dev/ttyS8","/dev/ttyS9","/dev/ttyS10","/dev/ttyS11",
                       "/dev/ttyS12","/dev/ttyS13","/dev/ttyS14","/dev/ttyS15","/dev/ttyUSB0",
                       "/dev/ttyUSB1","/dev/ttyUSB2","/dev/ttyUSB3","/dev/ttyUSB4","/dev/ttyUSB5",
                       "/dev/ttyAMA0","/dev/ttyAMA1","/dev/ttyACM0","/dev/ttyACM1",
                       "/dev/rfcomm0","/dev/rfcomm1","/dev/ircomm0","/dev/ircomm1",
						"/dev/ttyO0", "/dev/ttyO1", "/dev/ttyUSB0", "/dev/ttyO3", "/dev/ttyO4", "/dev/ttyO5"};



int RS232_OpenComport(int comport_number, int baudrate, int should_block)
{
  int baudr, status;

  if((comport_number>36)||(comport_number<0))
  {
  ...
  Cport[comport_number] = open(comports[comport_number], O_RDWR | O_NOCTTY  | O_NONBLOCK); //O_NDELAY | O_NONBLOCK
  ...
  new_port_settings.c_cc[VMIN] = should_block ? 1 : 0;;      /* block untill n bytes are received */
...
void RS232_FlushComport(int comport_number)
{
	usleep(10000);
...
int RS232_BytesAvailebleCount(int comport_number)
{
	int bytes_avail= 0;
	ioctl(Cport[comport_number], FIONREAD, &bytes_avail);
	return 0;
}
...
int RS232_PollComport_exact(int comport_number, char *buf, int size)
{
  int n = 0;
  int count = 0;
  struct timeval tv;
  ...
  do
  {
	  n = read(Cport[comport_number], buf+count, size-count);
	  if (n > -1){
		  count = count + n;
	  }
	  gettimeofday(&tv, NULL);
...
int RS232_SendBuf(int comport_number, const char *buf, int size)
{
